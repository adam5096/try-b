...
When(
  '我在{string}欄位填寫 {string}',
  async function (this: ICustomWorld, label: string, value: string) {
    const page = this.page!;
...
When(
  '我在{string}選擇 {string}',
  async function (this: ICustomWorld, label: string, optionText: string) {
    const page = this.page!;
    const labelRegex = new RegExp(`^${label}( \\*)?$`);
    await page
      .locator('.el-form-item', { has: page.locator('label', { hasText: labelRegex }) })
...
Then('我應該看到 {string} 的錯誤訊息', async function (this: ICustomWorld, message: string) {
  const page = this.page!;
  await expect(page.locator('.el-form-item__error', { hasText: message })).toBeVisible();
});
