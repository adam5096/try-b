...
Feature: 企業帳號註冊
  ...
  Scenario: 步驟一 - 使用有效的企業資料成功進入下一步
    Given 我在企業註冊頁面
    When 我在"企業名稱"欄位填寫 "火星科技股份有限公司"
    And 我在"統一編號"欄位填寫 "12345678"
    And 我在"帳號"欄位填寫 "testuser"
  ...
  Scenario: 步驟一 - 未填寫所有必填欄位無法進入下一步
    Given 我在企業註冊頁面
    When 我點擊 "下一步" 按鈕
    Then 我應該看到 "企業名稱為必填" 的錯誤訊息
    And 我應該看到 "帳號為必填" 的錯誤訊息
    And 我應該看到 "Email為必填" 的錯誤訊息
    And 我應該看到 "密碼為必填" 的錯誤訊息
    And 我應該看到 "確認密碼為必填" 的錯誤訊息
    And 我應該看到 "產業類別為必填" 的錯誤訊息
    And 我應該看到 "企業規模為必填" 的錯誤訊息
    And 我應該看到 "企業地址為必填" 的錯誤訊息
