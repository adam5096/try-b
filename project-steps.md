...
### EP8: 體驗者申請列表頁
- **頁面建構與資料呈現**：
  - 建立了特定計畫的「體驗者申請列表」頁面 (`pages/company/programs/[programId]/applicants.vue`)。
  - 根據設計稿，使用 Element Plus 的 `el-table` 元件，完成了「待審核」與「已審核」兩個申請者列表的切版與靜態資料渲染。
- **路由管理與健壯性**：
  - 在 `utils/companyRoutes.ts` 中新增了對應的路由輔助函數，並透過與使用者討論，統一了專案中路由物件的返回格式，提升了程式碼的可維護性。
  - 在 `layouts/company.vue` 中，採用 `router.resolve` 的最佳實踐來動態生成側邊欄連結，避免了硬編碼 URL。
  - 在修改共用路由檔案後，主動進行了全面的影響性分析，確保既有頁面的導航功能皆保持正常。
- **RWD 與版面修正**：
  - 修復了因 Flexbox 佈局壓縮導致的標題 (`h1`) 與篩選器標籤 (`span`) 過早換行問題。
  - 解決了因父層容器問題導致 `el-table` 內容無法正常渲染的錯誤，確保表格資料能正確顯示。

# 2025-07-28
### EP8: 體驗者申請列表頁 (續)
- **SSR 相容性偵錯**：
  - 遭遇了 Element Plus 的 `<el-select>` 元件在 Nuxt 3 環境下，`v-model` 雙向綁定完全失效的棘手問題。
  - 透過「隔離測試」與「減法測試」等專業除錯流程，最終定位問題根源為 `<el-select>` 元件與 `<el-table>` 元件在特定佈局下，存在著渲染衝突，破壞了 Vue 的響應式系統。
- **初步診斷結論**：透過隔離測試，確認了在乾淨的環境下 `<el-select>` 功能正常，證明問題出在 `applicants.vue` 頁面內部，極有可能是 `<el-select>` 與 `<el-table>` 之間的渲染衝突。
- **當前進度**：根本原因尚待釐清，解決方案仍在探索中。為了進行其他優先任務，目前已將本分支進度透過 `git stash` 暫存。