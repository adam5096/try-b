...
# 2025-08-18
### FEAT: 企業登入頁註冊導航
- **修正導航功能**：將 `pages/company/login.vue` 頁面中「立即註冊」的 `<a>` 標籤，替換為 Nuxt 的 `<NuxtLink>` 元件。
- **確保流程完整**：將連結指向 `/company/register`，修復了先前阻斷新企業用戶註冊的流程缺陷，確保使用者能從登入頁順利進入註冊流程。
- **遵循最佳實踐**：採用 `<NuxtLink>` 進行內部路由，以利用客戶端路由的效能優勢，避免不必要的頁面重載。

### FEAT: 企業註冊頁 UI 擴充
- **新增欄位**：根據 UI 設計稿，在註冊流程第一步 `components/company/register/Step1.vue` 中，新增了「帳號」、「Email」、「密碼」與「確認密碼」四個核心欄位。
- **更新資料模型**：同步在流程控制器 `pages/company/register.vue` 的 `formData` 狀態中，加入了對應的資料屬性。
- **遵循既有架構**：延續了專案的「容器/展示」元件模式，將 UI 變更限制在獨立的展示元件中，確保了修改的低耦合性與安全性。

# 2025-08-19
### MGT: 測試策略建立與開發流程定義
- **測試策略確立 (測試金字塔)**:
  - 確立了「測試金字塔」為指導原則，將測試重心從 E2E (BDD) 下移至元件測試 (TDD)，以追求更高的效率、穩定性與回饋速度。
  - 明確了 E2E 測試應專注於驗證高價值的「使用者旅程」，而元件測試則負責處理 UI 互動、邊界條件與 props/events 等所有實作細節。
- **BDD/TDD 協作流程定義**:
  - 採納了 BDD + TDD 的「雙環開發模式」，由 BDD (外循環) 定義高層級的業務目標與驗收標準，再由 TDD (內循環) 驅動元件的具體實作與品質保證。
- **BDD 結構重構**:
  - 為了提升可擴展性與可維護性，將 `features` 目錄依使用者角色 (`company`, `users`, `admin`) 進行了分類。
  - 遵循「慣例優於設定」原則，確認 `step_definitions` 應保持在頂層作為共享步驟庫，以促進步驟的重用性。
- **階段性開發策略 (應對時程壓力)**:
  - 為應對即將到來的發表會，採納了「先 BDD，後 TDD」的務實策略。
  - 此策略將優先完成可供演示的核心流程 (BDD)，並將 TDD 元件測試作為必須償還的「技術債」，於發表會後立即補強，以確保專案的長期健康。

### FEAT: 企業註冊 BDD 測試迭代與簡化
- **問題診斷與策略調整**: 針對 BDD 測試中反覆出現的時序問題 (Timing Issue) 與不穩定性 (Flakiness) 進行了除錯，並根據「優先交付核心價值」的原則，決定大幅簡化測試範圍。
- **BDD 規格重構**: 將 `company_register.feature` 從多個包含失敗路徑的複雜場景，重構為一個單一、端到端的「快樂路徑」測試，專注於驗證使用者能否成功完成整個註冊流程。
- **程式碼清理**: 同步簡化了 `step_definitions` 檔案，移除了所有不再需要的步驟定義，確保了測試程式碼與 BDD 規格的高度一致性與簡潔性。
- **達成穩定通過**: 經過簡化後，BDD 測試套件最終達成了穩定、快速 (~3 秒) 且 100% 通過的目標，為發表會的核心功能展示提供了可靠的品質保證。

---

### FEAT: 完成企業註冊流程閉環與 UX 優化

- **目標**：
  - 建立從註冊成功到引導登入的完整使用者流程閉環。
  - 提升表單提交時的使用者體驗，提供明確的操作反饋。

- **實作細節**：
  - **流程閉環 (Flow Completion)**：
    - 於 BDD 規格 `company_register.feature` 中，將快樂路徑延伸至「點擊按鈕，從成功頁導航至登入頁」。
    - 修改 `components/company/register/Step3.vue`，新增「前往登入頁面」按鈕，完成使用者旅程的最後一哩路。
  - **UX 優化 (UX Enhancement)**：
    - 修改 `components/company/register/Step2.vue`，為「註冊」按鈕新增 `loading` 狀態。當使用者點擊後、等待 API 回應期間，按鈕會顯示載入中且無法再次點擊。

- **測試策略與驗證**：
  - 執行 `pnpm test:bdd` 進行回歸測試，結果顯示包含 `loading` 模擬延遲的完整 19 個步驟全部通過。
  - 這證明了新功能不僅運作正常，也未對既有流程造成任何破壞。
  - 經討論確認，`loading` 狀態的具體 UI 驗證將下放至未來的「元件測試」中，以維持 E2E 測試的穩定性。
...
# 2025-08-20
### FEAT: 體驗者端驗證流程建構
- **架構複製與模組化**: 複製並調整企業端 (`company`) 的成功驗證模式，為體驗者端 (`user`) 快速建構了一套完整的驗證流程，包含 Pinia Store、後端 API 模擬與全域路由守衛。
- **UI 動態渲染**: 根據登入狀態 (`isLoggedIn`)，動態渲染 `layouts/user.vue` 中的導覽列，在「訪客」與「已登入」兩種模式間無縫切換。
- **Cookie 持久化除錯**:
  - 修正了因 `useCookie` 外層多餘的 `ref()` 封裝，導致響應式連結中斷、UI 狀態 cookie (`userAuthUser`) 無法寫入的問題。
  - 移除了在 `fetchUser` 函式中對 `HttpOnly` cookie 的客戶端讀取嘗試，此舉解決了因權限問題導致 API 請求被提前中斷的錯誤。
- **API 模擬**: 建立了 `/api/user/` 路徑下的 `login`、`logout`、`register`、`user` 四個端點，以支援完整的前端開發與測試流程。

### FIX: 路由守衛無限重新導向
- **問題診斷**: 將使用者首頁 (`/users`) 加入路由守衛的公開白名單後，已登入的使用者在被導向該頁面時，會觸發「重新導向至公開頁」的規則，導致無限重新導向迴圈。
- **邏輯重構**: 將公開頁面細分為「訪客專用頁」（如 `login`、`register`）與「通用公開頁」（如 `/users` 首頁）。
- **解決方案**: 修改 `middleware/user-auth.global.ts`，讓「已登入使用者重新導向」的規則僅對「訪客專用頁」生效，從而打破了重新導向迴圈，同時確保了所有公開頁面的可存取性。

### MGT: 驗證架構深度研討與最佳實踐確立
- **雙層防護模型確立**: 確立了「入口守衛 (路由中介軟體) + 資料室守衛 (API 攔截器)」的雙層安全模型，作為專案的驗證最佳實踐。
- **安全性與 UX 平衡**:
  - 釐清 `HttpOnly` Cookie 雖能有效防禦 XSS，但也造成前端無法主動偵測 Cookie 狀態的限制。
  - 承認在「手動刪除 Cookie」的極端場景下，目前架構存在一個短暫的「驗證空窗期」，直到下一次 API 請求觸發攔截器為止。
- **技術選型決策**:
  - 經權衡後，決定不採用輪詢 (Polling) 或 WebSocket 等高成本方案來彌補此空窗期。
  - 確認目前的「被動式、事件驅動」驗證模型，在安全性、使用者體驗與伺服器成本之間取得了業界公認的最佳平衡。
...
- **確立 Mock API 優先的開發流程**:
  - **決策**: 採納「契約先行」與「API 模擬」的開發模式。未來所有新功能在整合真實 API 前，都應先在 `server/api/` 目錄下建立對應的 Mock API。
  - **理由**: 此舉能讓前後端開發完全解耦並行，加速前端 UI 與商業 logique 的開發，並留下可供追溯、可作為備案的「活規格文件」。
- **釐清 Mock API 與真實 API 的關係**:
  - **闡明 `server/api/` 的角色**: 確認此目錄為 Nuxt 3 內建的「API 模擬層」，其檔案在部署至 Vercel 後會自動轉換為 Serverless Functions，可作為開發歷史紀錄與除錯備案。
  - **確認環境變數的隔離**: 明確了 Mock API (`/api/...`) 與真實 API (`NUXT_PUBLIC_API_BASE_URL`) 之間由環境變數 (`.env` vs Vercel 環境變數) 進行切換，兩者不會產生衝突，確保了不同環境的獨立性與靈活性。
...
### REFACTOR: 共用元件路徑遷移與全站修復
- **建立共用元件目錄**: 新增 `components/shared/` 目錄，用於存放跨功能模組（`admin`, `company`, `users`）共用的基礎元件，如各種 `Icon` 元件。
- **遷移並重構**: 將原先散落在 `components/` 根目錄下的所有 `Icon` 元件，全部遷移至新的 `shared` 目錄中。
- **全站引用路徑修復**:
  - 系統性地掃描整個專案，找出所有引用到舊元件名稱 (`<BetaIcon>`) 的檔案。
  - 全面更新 `pages/` 與 `layouts/` 目錄下所有相關檔案，將元件標籤修改為符合 Nuxt 3 自動引入規則的新名稱 (`<SharedBetaIcon>`)。
- **修復衍生問題**:
...
# 2025-08-24
### FEAT: 全站 SEO 與社群分享優化
- **建立全域 SEO 基礎**: 於 `app.vue` 中使用 `useSeoMeta` 建立全站的 SEO 預設值，包含動態標題範本 (`titleTemplate`)、網站描述與預設社群分享資訊 (`og:*`)，確保所有頁面皆有基礎設定。
- **客製化靜態頁面 SEO**:
  - 為首頁 (`pages/index.vue`) 與方案頁 (`pages/plan.vue`) 加入專屬的 `useSeoMeta` 設定，使其在搜尋與社群分享時能呈現更精準、更具吸引力的內容。
- **實作動態頁面 SEO**:
  - 重構方案詳情頁 (`pages/company/programs/[programId]/index.vue`)，導入 `useAsyncData` 來模擬 API 請求。
  - 成功將非同步獲取的資料（如計畫標題、描述）動態地綁定至 `useSeoMeta`，實現了內容與 SEO 資訊的同步。
- **迭代與優化文案策略**:
  - 深入探討 `description` (對搜尋引擎) 與 `ogDescription` (對社群) 的差異與目標受眾。
  - 根據「現實感」與「痛點解決」的核心原則，迭代並實作了三套不同風格的文案，最終為 `app.vue`、首頁、方案頁與詳情頁選擇了最能引發目標客群共鳴的文案版本。

### TECH: 建立技術性 SEO 基礎 (robots.txt, sitemap.xml)
- **建立 `robots.txt`**: 於 `public` 目錄下建立標準的 `robots.txt`，並加入指向網站地圖的路徑，同時刪除易造成混淆的 `_robots.txt` 備份檔。
- **釐清開發/生產環境差異**: 確認開發環境中由 Nuxt 動態生成的 `robots.txt` (`Disallow: /`) 是為防止測試站被索引的安全機制，與生產環境部署的版本不同。
- **自動化網站地圖**: 安裝並於 `nuxt.config.ts` 設定 `@nuxtjs/sitemap` 模組，使其能自動根據 `pages/` 目錄結構生成 `sitemap.xml`，提升搜尋引擎的索引效率。
- **安全性分析**: 深入探討了 `robots.txt` 與 `sitemap.xml` 可能被惡意掃描者利用的風險，並確立了防禦重點應在於應用程式本身的安全強化，而非依賴隱藏路徑。