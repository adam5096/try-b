...
<template>
  <section class="mx-auto max-w-container-users px-6 md:px-12 py-8 md:py-10">
    ...
    <div class="flex items-start justify-between gap-4">
      <div>
        <h2 class="text-2xl md:text-3xl font-bold text-gray-800">我的收藏清單</h2>
        <p class="mt-2 text-gray-500">管理您收藏的體驗計畫，查看詳情或直接申請參與</p>
      </div>

      <el-popconfirm title="確定清空收藏？" confirm-button-text="清空" cancel-button-text="取消" @confirm="handleClearAll">
        <template #reference>
    ...
    <!-- Empty -->
    <el-empty v-if="total === 0" description="尚未收藏任何體驗" />

    <!-- Grid -->
    <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
      <el-card v-for="item in visibleItems" :key="item.id" shadow="hover">
        <!-- Image placeholder with favorite icon -->
        <div class="relative">
          <div class="h-56 bg-gray-200 flex items-center justify-center text-3xl text-gray-500 select-none">圖片</div>
          <button
          ...
          >
            <font-awesome-icon :icon="[item.isFavorited ? 'fas' : 'far', 'heart']" class="h-5 w-5 text-gray-700" />
          </button>
        </div>
        ...
        <div class="mt-4">
          ...
          <p class="mt-2 text-gray-600 leading-relaxed line-clamp-3">{{ item.description }}</p>
          ...
          <div class="mt-4">
            <el-button class="w-full" @click="handleViewDetail(item)">查看詳情</el-button>
          </div>
        </div>
      </el-card>
    </div>
