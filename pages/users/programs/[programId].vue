	...
	<main class="min-h-screen bg-brand-gray">
		<div class="mx-auto max-w-container-users px-6 py-12 md:px-12">
			...
			<div class="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
				<h1 class="w-full text-2xl font-bold text-primary-blue-dark">
					體驗詳情
				</h1>
			...
			>
						...
						<div class="space-y-8 text-gray-800">
							...
							<section v-if="parsedIntro?.requirements && parsedIntro.requirements.length > 0">
								...
								<ol class="list-decimal space-y-1 pl-6">
									<li
										v-for="requirement in parsedIntro.requirements"
										:key="requirement"
									>
										{{ requirement }}
									</li>
							...
							<section v-if="parsedIntro?.preparation && parsedIntro.preparation.length > 0">
								...
								<ol class="list-decimal space-y-1 pl-6">
									<li
										v-for="item in parsedIntro.preparation"
										:key="item"
									>
										{{ item }}
									</li>
				...
				>
					<div class="rounded-lg border border-gray-200 bg-white p-8 shadow-sm">
						...
						<div class="mb-6">
							...
							<dl
								v-if="programDetail.Steps && programDetail.Steps.length > 0"
								class="space-y-4"
							>
								<div
									v-for="(step, idx) in programDetail.Steps"
									:key="idx"
									class="grid grid-cols-12 gap-4"
								>
									<dt class="col-span-12 truncate whitespace-nowrap font-semibold text-gray-700 md:col-span-2">
										{{ step.Name }}
									</dt>
						...
						<div>
							...
							<SharedGoogleMapEmbed
								:src="programDetail.address_map || ''"
								empty-text="尚未提供地圖資訊"
							/>
						</div>
					</div>
				...
				>
					<div class="rounded-lg border border-gray-200 bg-white p-8 shadow-sm">
						...
						<div
							v-if="programDetail.Images && programDetail.Images.length > 0"
							class="grid grid-cols-1 gap-6 md:grid-cols-2"
						>
							<div
								v-for="(image, idx) in programDetail.Images"
								:key="idx"
							...
							>
								...
								<div
									v-else
									class="flex h-48 w-full items-center justify-center text-3xl text-gray-700"
								>
									圖片
								</div>
							</div>
