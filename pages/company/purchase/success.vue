...
const queryPaymentStatusFromASP = async (): Promise<PaymentResultResponse | null> => {
	try {
		...
		if (hasTradeInfo.value) {
			...
			// 嘗試從 URL 參數或其他地方取得 OrderNo，如果沒有則使用預設值
			const orderNoToUse = orderNum.value || '';
...
const testPaymentResultAPI = async () => {
	try {
		...
		// 使用規格書中的測試參數
		const testOrderNo = 'ORD20251010001';
...
const testManualParameters = async () => {
	...
	try {
		...
		// 直接呼叫結帳結果 API - 需要 OrderNo 參數
		const orderNoToUse = orderNum.value || 'ORD20251010001';
